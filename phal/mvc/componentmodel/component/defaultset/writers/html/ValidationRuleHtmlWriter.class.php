...
class __ValidationRuleHtmlWriter extends __ComponentWriter {
    ...
    private function _getValidFormatJsPart(__IComponent &$component) {
        ...
        if($valid_format != null) {
            $error_message = $component->getErrorMessageIfNotValid();
            if($error_message != null) {
                $return_value = $component->getId() . '.add(Validate.Format, { pattern: /' . $valid_format . '/i, failureMessage : "' . $error_message . '"});' . "\n";
            }
            else {
    ...
    private function _getMandatoryJsPart(__IComponent &$component) {
        ...
        if($component->getMandatory()) {
            $error_message = $component->getErrorMessageIfEmpty();
            if($error_message != null) {
                $return_value = $component->getId() . '.add(Validate.Presence, {failureMessage : "' . $error_message . '"});' . "\n";
            }
            else {
    ...
    private function _getAcceptanceJsPart(__IComponent &$component) {
        ...
        if($component->getAcceptance()) {
            $error_message = $component->getErrorMessageIfEmpty();
            if($error_message != null) {
                $return_value = $component->getId() . '.add(Validate.Acceptance, {failureMessage : "' . $error_message . '"});' . "\n";
            }
            else {
    ...
    private function _getListenValidateEvent(__IComponent &$component) {
        $component_to_validate = $component->getComponentToValidate();
        $return_value = 'document.observe("phal:validate", function(event) { if(event.target.id == "' . $component_to_validate->getId() . '") { ' . $component->getId() . '.removeMessageAndFieldClass(); event.validationResult = ' . $component->getId() . '.validate(); } } );' . "\n";
        return $return_value;
    }
