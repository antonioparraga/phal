...
class __ErrorHandler {
    ...
    public function getErrorPrinter() {
        ...
        if(__Phal::getInstance()->getStatus() == __Phal::STATUS_RUNNING) {
            $request_type = __Client::getInstance()->getRequestType();
            switch ($request_type) {
            	case REQUEST_TYPE_XMLHTTP:
    ...
    public function handleException(Exception $exception)
    {
        ...
        //clear the response (if exists)
        if(__ApplicationContext::getInstance() != null) {
        ...
        try {
            ...
            if ( $exception instanceof __PhalException ) {
    ...
    public function logException(Exception $exception) {
        if(class_exists('__CurrentContext') && __CurrentContext::getInstance() != null) {
            ...
            if($phal_runtime_directives->hasDirective('LOG_EXCEPTIONS') && 
               $phal_runtime_directives->getDirective('LOG_EXCEPTIONS')) {
                if ( $exception instanceof __PhalException ) {
                ...
                if($phal_runtime_directives->hasDirective('LOG_DEBUG_INFO') && 
                   $phal_runtime_directives->getDirective('LOG_DEBUG_INFO')) {
                   $log_message .= "\n<exception>";
...
class __HtmlErrorPrinter implements __IErrorPrinter {
    ...
    public function displayError(Exception $exception) {
        ...
        if(__Phal::getInstance()->getRuntimeDirectives()->getDirective('DISPLAY_EXCEPTIONS')) {
        ...
        if(__Phal::getInstance()->getRuntimeDirectives()->getDirective('DEBUG_MODE') &&
           __Phal::getInstance()->getRuntimeDirectives()->getDirective('DISPLAY_DEBUG_INFO')) {
